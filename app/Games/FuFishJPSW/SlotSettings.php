...
use VanguardLTE\Http\Traits\ReportTurnOverTraits;
use VanguardLTE\Http\Traits\ReportGameTraits;
    ...
    class SlotSettings
    {
        ...
        use ReportTurnOverTraits;
        use ReportGameTraits;
                ...
                if( $this->jpgs[$i]->get_pay_sum() < $jsum[$i] && $this->jpgs[$i]->get_pay_sum() > 0 ) 
                {
                    ...
                    else
                    {
                        ...
                        if( $this->jpgs[$i]->get_pay_sum() > 0 ) 
                        {
                            ...
                            $betAmount = 0;
                            $winAmount = $this->jpgs[$i]->get_pay_sum();
                            $gameName = $this->game->name . ' JPG ' . $this->jpgs[$i]->id;
                            $this->makeReporTurnOverForEGame($gameName, $betAmount, $winAmount);
                            $this->makeReportForEGame($gameName, $this->playerId, $betAmount, $winAmount, 0);
        ...
        public function SaveLogReport($spinSymbols, $bet, $lines, $win, $slotState)
        {
            ...
            $betAmount = $bet * $this->CurrentDenom;
            $winAmount = $win * $this->CurrentDenom;
            $this->makeReporTurnOverForEGame($reportName, $betAmount, $winAmount);
            $this->makeReportForEGame($reportName, $this->playerId, $betAmount, $winAmount, 0);
