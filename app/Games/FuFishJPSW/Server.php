                ...
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        switch( $aid ) 
                        {
                            ...
                            case 'jp-ticker':
                                $balanceInCents = sprintf('%01.2f', $slotSettings->GetBalance());
                                $result_tmp[0] = '42["jp-ticker",{"balance":{"currency":"","amount":' . $balanceInCents . ',"real":{"amount":' . $balanceInCents . '},"bonus":{"amount":0}},"tickers":[{"jackpotId":"0","pools":{ "big":{"amount":' . ($slotSettings->slotJackpot[0] * 100) . '},"small":{"amount":' . ($slotSettings->slotJackpot[1] * 100) . '},"medium":{"amount":' . ($slotSettings->slotJackpot[2] * 100) . '} } }],"result":{"request":"jp-ticker"},"roundEnded":false}]';
                                break;
                            case 'balance':
                                $balanceInCents = sprintf('%01.2f', $slotSettings->GetBalance());
