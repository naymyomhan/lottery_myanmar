...
class HistoryController extends Controller
{
    ...
    public function getLive()
    {
        try {
            $today = Carbon::today();
            $data = TwoDLive::whereDate('created_at', $today)->get(['set', 'value', 'time', 'twod']);

            return $this->success("Get 2D Live successful", $data);
        } catch (\Throwable $th) {
    ...
    public function getTodayHistory()
    {
        try {
            $today = Carbon::today();
            $tdhistory = TwoDHistory::whereDate('created_at', $today)->get(['set', 'value', 'open_time', 'twod', 'created_at']);
